...
> 對象：專案三位成員。目的：讓每個人**完全理解目前做到哪、還有什麼沒做、
> 怎麼上手 git 與功能開發、以及分工**。
> 最後更新：2026-09-08（by @ChichiTung）
>
> 搭配閱讀：[ONBOARDING.md](./ONBOARDING.md)（環境建置）、
> [GOVERNANCE.md](./GOVERNANCE.md)（權限設定）、
> 根目錄 [CLAUDE.md](../CLAUDE.md)（不可違反的領域規則）、[AGENTS.md](../AGENTS.md)（開發流程）。

---

## 1. 這個專案是什麼

疊加在既有費用系統之上的 **AI 費用單據初審 Agent**。Agent 讀單據、比對申報與單據、
跑合規規則，把每筆案件分成 **建議通過 / 建議補件 / 建議人工審核** 三桶，
**最終決定永遠是人做的**（Agent 不下最終決定）。

- 目前只做 **M1 Review Copilot**（M1-U1 財務初審、M1-U2 主管稽核）。
- **M2（風險情報）、M3（自動處置）不在本階段範圍。** 看到 M2/M3 的東西寫進
  `openspec/backlog.md`，不要順手做。
...
這階段完成的是**地基與骨架**，不是功能。誠實的完成度：

| 層                                | 狀態            | 完成度 | 說明                                                           |
| --------------------------------- | --------------- | ------ | -------------------------------------------------------------- |
| 資料模型 + DB + 治理約束          | ✅ 完成         | ~95%   | schema v3、migration、trigger/CHECK/EXCLUDE 全部套用並實機驗證 |
| shared 型別契約                   | ✅ 完成         | ~90%   | enum 詞彙、disposition 矩陣、parity/matrix 測試                |
| seed 示範資料                     | 🟡 骨架         | ~35%   | 4 個代表案件 + 1 個 R7 參照案件跑通，尚未擴充到 10 情境        |
| **後端審核引擎（規則/比對/API）** | ❌ 未開始       | ~5%    | 只有 NestJS 樣板，無 review 模組、無 API                       |
...
- **治理保證（實機驗證擋下）**：
  - `AuditEvent`/`Disposition`/`SupervisorReview` append-only（UPDATE/DELETE/TRUNCATE 都被 trigger 擋）。
  - 非通過的 `RuleResult`/`MatchResult` 沒掛 `Evidence` → DB 拒絕寫入。
  - GATED/ABSTAIN 沒附原因 → 拒絕；Policy 生效區間不重疊（EXCLUDE）。
- **shared 契約**：四分類、三桶建議、五態結果、reviewer/supervisor 動作 enum，全部對齊 Prisma；
  `disposition.ts` 是唯一的合法動作矩陣，前後端都 import 這份。
- **seed**：`EXP-2026-2001`(NORMAL)、`2002`(EXCEPTION/R7)、`2003`(MISSING/R4)、`2004`(HUMAN/abstain)
  - `EXP-2026-1043`(R7 參照，REVIEW_CLOSED)。全部含 AuditEvent hash chain。
- **文件與治理**：ONBOARDING、GOVERNANCE、CODEOWNERS（已指到 @ChichiTung）。
- **CI 四關**：`pnpm lint && pnpm typecheck && pnpm test && pnpm build` 全綠。

### ❌ 尚未開發（M1 待做的主體）

- **後端審核引擎**：規則 handler（R1~R10）、一致性比對、雙假設評估、分類與建議產生、
  `POST /cases/:id/runs`（非同步、回 202+runId）等 API。
...
- **enum-parity（8）**：讀 `schema.prisma` 的 enum，跟 `enums.ts` 的 Zod enum 逐一比對，
  確保「資料庫 ↔ 前後端共用型別」100% 一致。任一邊改 enum 沒同步，這關就紅。
- **disposition（6）**：測合法動作矩陣（非法動作被拒、`MANUAL_REVIEW+ACCEPT` 會 escalate 等）。

**目前沒有審核引擎的測試、沒有 mock 案件資料在測試裡、沒有連 DB 的整合測試** —— 因為審核引擎還沒寫。
`apps/api` 的 `app.controller.spec.ts` 只是 NestJS 樣板（"Hello World"），非業務邏輯。
...
2. **先開 OpenSpec proposal 再寫程式**（非瑣碎改動）：`openspec new change "<change-id>"`，
   依序補 proposal → specs → design → tasks（見 AGENTS.md / CLAUDE.md）。
...
`schema.prisma`、`prisma/migrations/`、`packages/shared/src/domain/`、`CLAUDE.md`、
`AGENTS.md`、`.claude/`、`CODEOWNERS`、`openspec/config.yaml`。要改先開 issue/proposal。
...
前提：`schema.prisma` 與 `packages/shared/src/domain/` 由 **@ChichiTung**（schema owner）掌關，
任何要改這兩處的需求先提 issue。以下分工讓三人可**平行、低衝突**開發。

### 👤 成員 A — 後端審核引擎（owner: @ChichiTung 或指派）
...
> **以舊版排版為基底**（側邊主選單 + 案件總覽儀表板 + 檢查清單條列）；
> v0.3 的 AI 內部判別 tag（DepChip、§條號、信心度、五態）**不進前端**。
...
1. **seed 擴充**：把骨架 4 案補到完整 10 情境（照 `.claude/skills/demo-case`），
   含 R8 拆單、R10 加總、低信心欄位、未匹配單據等（tasks §4.2/4.4/4.5）。
...
- A 與 B 的交界是 **API 契約**：建議 A 先把 `POST /cases/:id/runs` 與案件查詢的
  **回傳型別**放進 `packages/shared`（先問 owner），B 就能照型別開發、不必等 API 實作完。
...
`bootstrap-m1-foundation` 這個 OpenSpec change 目前 **17/22 tasks 完成**。剩餘 5 項：

- `2.3` hash chain 竄改偵測測試
- `4.2` seed 擴充到 10 情境
- `4.4` 低信心欄位案件
- `4.5` db:reset 兩次一致性驗證
- `6.2` app 端到端驗證

完整清單見 `openspec/changes/bootstrap-m1-foundation/tasks.md`。
這 5 項已分配給上面的成員 A/C，可作為各自的第一批任務。
